des2022path <- fs::path(scriptPath_UnsupAnalysis, "Panel 2")
...
flowSomResPath <- fs::path(scriptPath_Data,  "Panel 2")
...
temp <- read.csv2(fs::path("F:", "Forskningsprosjekter", "PDB 2794 - Immune responses aga_", "Forskningsfiler", "JOBO", "CyTOF", "Analyse i R OUS", "CleanUpGatingMarch2022", "Analyse", "Endelig", "Cytof unsupperviced", "Panel 2", "All", "seed 2234", "antallPerClusterOgPat_k_10_seed2234All.csv"))
...
for(i_like in 1:length(like)){
 # i_like <- 23
   Cluster <- like[[i_like]]
   write.table(Cluster, fs::path(scriptPath_UnsupAnalysisUt, paste0("like_klustre_panel2_", i_like, ".txt")))
 if(length(Cluster) > 1){
  params <- list()
  ...
  rmarkdown::render(fs:::path(scriptPath_UnsupAnalysis , "resultater_Regression_negbin_rapporter_alle.Rmd"), output_file = fs::path(params$utflowSomResPath,paste0("like_klustre_panel2_", i_like, ".docx")))
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, paste0("like_klustre_panel2_", i_like, ".tiff")), width = 1350, height = 900)
    print(g)
  dev.off()
