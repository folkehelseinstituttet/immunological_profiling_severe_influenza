...
j <- 1
    ...
    temp_sign <- data.frame(X = tall, sign = Cluster)
...
#  d$Group <- factor(d$Group, levels = rev(names(color_group)))
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P2_marker_plot_alle.tiff"), width = 1350, height = 1300)
  print(g)
  dev.off()
  
  
  
  color_group_mindre <- unique(color_group)
  names(color_group_mindre) <- c("CD4", "CD8", "NK", "Th17")
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P2_marker_plot_alle_legend.tiff"), width = 350, height = 300)
  print(gleg)
  dev.off()
