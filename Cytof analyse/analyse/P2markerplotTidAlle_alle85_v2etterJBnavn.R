...
stiDes2022 <- fs::path("F:", "Forskningsprosjekter", "PDB 2794 - Immune responses aga_", "Forskningsfiler", "JOBO", "CyTOF", "Analyse i R OUS", "CleanUpGatingMarch2022", "Analyse", "Endelig", "Cytof unsupperviced", "Endelig_des2022",  "Panel 2")
sti <- fs::path("F:", "Forskningsprosjekter", "PDB 2794 - Immune responses aga_", "Forskningsfiler", "JOBO", "CyTOF", "Analyse i R OUS", "CleanUpGatingMarch2022", "Analyse", "Endelig", "Cytof unsupperviced",   "Panel 2")


gates <- read.csv2(fs::path("F:", "Forskningsprosjekter", "PDB 2794 - Immune responses aga_", "Forskningsfiler", "JOBO", "CyTOF", "Analyse i R OUS", "CleanUpGatingMarch2022", "gating_results_panel2_mars2022", "posNeg", "Data", "gater.csv"))
navn <- read.csv2( fs::path("F:", "Forskningsprosjekter", "PDB 2794 - Immune responses aga_", "Forskningsfiler", "JOBO", "CyTOF", "Analyse i R OUS", "CleanUpGatingMarch2022", "gating_results_panel2_mars2022", "posNeg", "Data", "navn.csv" ))
...
# signif <- readxl::read_excel(fs::path(scriptPath_UnsupAnalysis, "Endelig_des2022", "Figurer", "15122022 PANEL2_ST1vsMT1VsCST2vsMT2VsC_FINAL.xlsx"))
# signif$`Anjas navnPercent`  <- gsub("All_", "", signif$`Anjas navnPercent`)
# signif <- signif[signif$`ta med` == 1, ]
# signif <- signif[!is.na(signif$navn_med_prosent),]
# tall <- as.numeric(signif$`Kluster nr`)
# signif <- signif[order(tall),]
# Cluster <- signif$`Anjas navnPercent`
# ClusterKort <- gsub("kluster_", "", gsub("seed_2234_k_", "", Cluster))
# signif$Navn_markerplot <- signif$navn_med_prosent 
# groups <- signif$Navn_markerplot



color_group_mindre <- c("#33CCCC", "#1F78B4",   "#FB9A99", "#FDBF6F","#B2DF8A", "#CAB2D6", "#f708db", "#fd028f",   "#CCCCCC")
names(color_group_mindre) <- c("CD4", "CD8", "gdT", "B",  "NK", "NKT", "Th17", "Treg", "NA")

#levels = c("CD4", "CD8",   "gdT",  "B", "NK", "NKT",  "Th17", "Treg", "NA" )

signif <- read.csv2(fs::path(scriptPath_UnsupAnalysis, "Endelig_des2022", "Figurer", "klusternavnP2 – JB.csv"))
ClusterKort <- signif$Anja_navn
Cluster <- paste0("seed_2234_k_", gsub("_", "_kluster_", ClusterKort))
signif$Navn_markerplot <- paste(signif$Anja_navn, signif$kort_navn)
signif$Navn_markerplot <- gsub(" NA", "", signif$Navn_markerplot)
signif$kort_navn <- factor(signif$kort_navn, levels = c("CD4", "CD8",   "gdT",  "B", "NK", "NKT",  "Th17", "Treg", "NA" ))#, "Tregs")))  #"DC",  "#33A02C"; "MAIT" "#FB9A99"; "Mo" "#FF7F00"
signif$colour <- "#CCCCCC"
signif$colour[signif$kort_navn == "CD4"] <-  color_group_mindre["CD4"] # "#33CCCC"
signif$colour[signif$kort_navn == "CD8"] <-  color_group_mindre["CD8"] # "#1F78B4"
signif$colour[signif$kort_navn == "NK"] <-  color_group_mindre["NK"] # "#B2DF8A"
signif$colour[signif$kort_navn == "gdT"] <-  color_group_mindre["gdT"] # "#FB9A99"
signif$colour[signif$kort_navn == "B"] <-  color_group_mindre["B"] # "#FDBF6F"
signif$colour[signif$kort_navn == "NKT"] <-  color_group_mindre["NKT"] # "#CAB2D6"
signif$colour[signif$kort_navn == "Th17"] <-  color_group_mindre["Th17"] # "#f708db"
signif$colour[signif$kort_navn == "Treg"] <-  color_group_mindre["Treg"] # "#fd028f"

signif <- signif[order(signif$kort_navn),]

groups <- signif$Navn_markerplot


#levels = c("CD4", "CD8", "NK", "DC", "gdT", "MAIT", "B", "Mo", "NKT"))#, "Tregs"))
...
j <- 1
    ...
    taMed <- colnames(temp_unike[grep("seed_2234", colnames(temp_unike))])
    
    temp_sign <- data.frame(1:length(taMed), taMed)  #  data.frame(X = tall, sign = Cluster)
    colnames(temp_sign) <- c("X", "sign")
    temp <-  temp_unike[, grep("seed_2234", colnames(temp_unike)) ]# as.character(temp_sign$sign)]
    if(!is.null(dim(temp))){
...
q5 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_2234_k_", "", rownames(q5_all))),  q5_all)
q10 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_2234_k_", "", rownames(q10_all))),  q10_all)
q25 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_2234_k_", "", rownames(q25_all))),  q25_all)
q50 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_2234_k_", "", rownames(q50_all))),  q50_all)
q75 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_2234_k_", "", rownames(q75_all))),  q75_all)
q90 <- data.frame(Group = gsub( "kluster_", "", gsub("All_seed_2234_k_", "", rownames(q90_all))),  q90_all)
q95 <- data.frame(Group = gsub( "kluster_", "", gsub("All_seed_2234_k_", "", rownames(q95_all))),  q95_all)
  ...
  d <- merge(d, signif[, c("Anja_navn", "Navn_markerplot")], by.x = "Group", by.y = "Anja_navn")
   
   
   color_group <- rep("#CCCCCC", nrow(q5))
   names(color_group) <- q5$Group
   
   color_group[signif$Anja_navn] <- signif$colour
   
   # 
   # color_group <- c(signif$colour)
   # names(color_group) <- c( signif$Navn_markerplot)
   #   
...
#  d$Group <- factor(d$Group, levels = rev(names(color_group)))
  ...
  d$marker_short_name <- factor( d$marker_short_name, levels = c("CCR7", "CD3", "CD4", "CD8", "CD14", "CD16", "CD19", "CD25", "CD27", "CD28", 
                                                                 "CD33", "CD38", "CD44", "CD45", "CD45RA", "CD56", "CD57",      
                                                                 "CD107a", "CD127", "CD137", "CD154", "CD223", "CD272", "CTLA-4", "CXCR5", "FoxP3",   
                                                                 "GM-CSF", "GranzB", "HLADR", "IFNg", "IL-1b", "IL-2", "IL-6", "IL-10", 
                                                                 "IL-12p70", "IL-17A", "MIP-1b", "PD1", "PD-L1", "PD-L2", "Perforin", "TCRgd","TIM-3",
                                                                 "TNFa"))
  
  
  #unique_col <- levels(d$col)
  
  #d$Group <- factor(d$Group, rev(c( as.character(groups))))
  d$Group <- factor(d$Group, rev(signif$Anja_navn))
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P2_marker_plot_alle_alle85medFarge.tiff"), width = 1350, height = 3000)
  print(g)
  dev.off()
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P2_marker_plot_alle85medFarger_legend.tiff"), width = 350, height = 300)
  print(gleg)
  dev.off()
  # 
  # 