...
signif <- readxl::read_excel(fs::path(scriptPath_UnsupAnalysis, "Endelig_des2022", "Figurer", "PANEL2_ST1vsMT1_FINAL.xlsx"))
signif <- signif[order(signif$`Kluster nr`),]
Cluster <- signif$`Anjas navnPercent`
signif$Navn_markerplot <- signif$Navn_V2
...
q5 <- data.frame(Group = groups,  q5_all[Cluster,])
q10 <- data.frame(Group = groups,  q10_all[Cluster,])
q25 <- data.frame(Group = groups,  q25_all[Cluster,])
q50 <- data.frame(Group = groups,  q50_all[Cluster,])
q75 <- data.frame(Group = groups,  q75_all[Cluster,])
q90 <- data.frame(Group = groups,  q90_all[Cluster,])
q95 <- data.frame(Group = groups,  q95_all[Cluster,])
...
q95 <- data.frame(Group = c("Cl0 (Other cells, ns)", as.character(q95$Group)), rbind(q0[7,-1], q95[,-1]))
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P2_marker_plot.tiff"), width = 1350, height = 900)
  print(g)
  dev.off()
