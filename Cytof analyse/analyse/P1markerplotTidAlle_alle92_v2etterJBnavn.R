...
# 
# signif <- readxl::read_excel(fs::path(scriptPath_UnsupAnalysis, "Endelig_des2022", "Figurer", "151222_Panel1_ST1vsMT1VsCST2vsMT2VsC_FINAL.xlsx"))
# 
...
# ClusterKort <- gsub("kluster_", "", gsub("seed_1345_k_", "", Cluster))
...
signif <- read.csv2(fs::path(scriptPath_UnsupAnalysis, "Endelig_des2022", "Figurer", "klusternavnP1_JB.csv"))
ClusterKort <- signif$Anja_navn
Cluster <- paste0("seed_1345_k_", gsub("_", "_kluster_", ClusterKort))
signif$Navn_markerplot <- paste(signif$Anja_navn, signif$kort_navn)
signif$Navn_markerplot <- gsub(" NA", "", signif$Navn_markerplot)
signif$kort_navn <- factor(signif$kort_navn, levels =c("CD4", "CD8", "MAIT", "gdT", "B", "plasma cells", "NK", "NKT", "Th17", "Treg", "Mo","DC", "Neutrophil","NA"))#, "Tregs")))  #  
signif$colour <- "#CCCCCC"
signif$colour[signif$kort_navn %in% "CD4"] <-  color_group_mindre["CD4"] # "#33CCCC"
signif$colour[signif$kort_navn %in% "CD8"] <-  color_group_mindre["CD8"] # "#1F78B4"
signif$colour[signif$kort_navn %in% "NK"] <-  color_group_mindre["NK"] # "#B2DF8A"
signif$colour[signif$kort_navn %in% "gdT"] <-  color_group_mindre["gdT"] # "#FB9A99"
signif$colour[signif$kort_navn %in% "B"] <-  color_group_mindre["B"] # "#FDBF6F"
signif$colour[signif$kort_navn %in% "NKT"] <-  color_group_mindre["NKT"] # "#CAB2D6"
signif$colour[signif$kort_navn %in% "Th17"] <-  color_group_mindre["Th17"] # "#f708db"
signif$colour[signif$kort_navn %in% "Treg"] <-  color_group_mindre["Treg"] # "#fd028f"
signif$colour[signif$kort_navn %in% "MAIT"] <-  color_group_mindre["MAIT"] 
signif$colour[signif$kort_navn %in% "Mo"] <-  color_group_mindre["Mo"] 
signif$colour[signif$kort_navn %in% "DC"] <-  color_group_mindre["DC"] 
signif$colour[signif$kort_navn %in% "plasma cells"] <-  color_group_mindre["plasma cells"] 
signif$colour[signif$kort_navn %in% "Neutrophil"] <-  color_group_mindre["Neutrophil"] 
...
taMed <- colnames(temp_unike[grep("seed_1345", colnames(temp_unike))])
    
      temp_sign <- data.frame(1:length(taMed), taMed)  #  data.frame(X = tall, sign = Cluster)
      colnames(temp_sign) <- c("X", "sign")
      temp <-  temp_unike[, grep("seed_1345", colnames(temp_unike)) ]# as.character(temp_sign$sign)]
      if(!is.null(dim(temp))){
...
q5 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_1345_k_", "", rownames(q5_all))),  q5_all)
q10 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_1345_k_", "", rownames(q10_all))),  q10_all)
q25 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_1345_k_", "", rownames(q25_all))),  q25_all)
q50 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_1345_k_", "", rownames(q50_all))),  q50_all)
q75 <- data.frame(Group = gsub("kluster_", "", gsub("All_seed_1345_k_", "", rownames(q75_all))),  q75_all)
q90 <- data.frame(Group = gsub( "kluster_", "", gsub("All_seed_1345_k_", "", rownames(q90_all))),  q90_all)
q95 <- data.frame(Group = gsub( "kluster_", "", gsub("All_seed_1345_k_", "", rownames(q95_all))),  q95_all)
...
#  
  ...
  if(!is.null(gates)){
   ...
   # color_group[ClusterKort] <- signif$colour
   #  
...
#d$Group <- factor(d$Group, rev(q5$Group))
d$Group <- factor(d$Group, rev(signif$Anja_navn))
  ...
  g <- ggplot(d, aes(x = q50, y = Group, xmin = q10, xmax = q90)) + #, col = Group)) +
    ...
    facet_wrap(vars(marker_short_name), ncol = 11, scale = "free_x") + theme_classic(base_size = 15) +
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P1_marker_plot_alle_alle92medFarge.tiff"), width = 1350, height = 3400)
  print(g)
  dev.off()
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P1_marker_plot_alle92medFarger_legend.tiff"), width = 350, height = 300)
  print(gleg)
  dev.off()
