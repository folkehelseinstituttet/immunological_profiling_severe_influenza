...
d$marker_short_name <- factor(d$marker_short_name, levels = c("CCR4", "CCR6", "CCR7", "CD3", "CD4", "CD5", "CD8",   
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P1_marker_plot_alle.tiff"), width = 1350, height = 1300)
  print(g)
  dev.off()
  ...
  names(color_group_mindre) <- c("CD4", "CD8", "MAIT", "B", "plasma cells", "NK", "NKT", "Mo", "DC")
  ...
  g2 <- ggplot(dtemp, aes(x = x, y = y, col = cellname)) + 
    ...
    scale_colour_manual(values = as.character(color_group_mindre), breaks = names(color_group_mindre), 
                        limits =  c("CD4", "CD8", "MAIT", "B", "plasma cells", "NK", "NKT", "Mo", "DC"))  +
  ...
  tiff( fs::path(scriptPath_UnsupAnalysisUt, "P1_marker_plot_alle_legend.tiff"), width = 350, height = 300)
  print(gleg)
  dev.off()
